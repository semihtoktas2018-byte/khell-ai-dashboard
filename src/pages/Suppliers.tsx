...
export default function Suppliers() {
  ...
  const filtered = useMemo(() => {
    ...
    items = [...items].sort((a, b) => {
      ...
      if (sortKey === "totalCost") {
        aVal = a.price + a.shippingCost;
        bVal = b.price + b.shippingCost;
      } else {
        aVal = a[sortKey];
        bVal = b[sortKey];
      }
  ...
  // Best supplier = lowest total cost with rating >= 4.5
  ...
  const SortHeader = ({ label, sortField }: { label: string; sortField: SortKey }) => (
    <th
      className="text-right py-3 px-5 text-xs font-medium text-muted-foreground cursor-pointer hover:text-foreground transition-colors select-none"
      onClick={() => handleSort(sortField)}
    >
      <span className="inline-flex items-center gap-1">
        {label}
        {sortKey === sortField && <ArrowUpDown className="h-3 w-3 text-primary" />}
      </span>
    </th>
  );
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1 max-w-md">
          ...
          <input type="text" placeholder="Tedarikçi veya ürün ara..." value={query} onChange={(e) => setQuery(e.target.value)} className="input-dark w-full pl-10" />
        </div>
        <div className="flex gap-2">
          {sources.map((s) => (
            <button key={s} onClick={() => setSource(s)} className={`px-3 py-2 text-xs font-medium rounded-lg transition-colors whitespace-nowrap ${source === s ? "bg-primary text-primary-foreground" : "bg-card border border-border text-muted-foreground hover:text-foreground"}`}>
              {s}
            </button>
          ))}
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                ...
                <SortHeader label="Fiyat" sortField="price" />
                <SortHeader label="Kargo" sortField="shippingCost" />
                <th className="text-left py-3 px-5 text-xs font-medium text-muted-foreground">Teslimat</th>
                <SortHeader label="Min. Sipariş" sortField="minOrder" />
                <SortHeader label="Puan" sortField="rating" />
                <SortHeader label="Toplam" sortField="totalCost" />
              ...
              {filtered.map((s, i) => {
                ...
                return (
                  <motion.tr key={s.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }} className={`border-b border-border/50 transition-colors ${isBest ? "bg-winning/5" : "hover:bg-accent/30"}`}>
                    ...
                    <td className="py-3 px-5 text-right font-mono tabular-nums text-foreground font-medium">${(s.price + s.shippingCost).toFixed(2)}</td>
