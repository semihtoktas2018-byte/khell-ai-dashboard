...
export default function TrendingProducts() {
  ...
  const fetchTrending = useCallback(async () => {
    ...
    setError(null);
    try {
      const randomTerms = [...TREND_SEARCH_TERMS].sort(() => Math.random() - 0.5).slice(0, 5);
  ...
  const visibleItems = isPro ? items : items.slice(0, effectiveLimit);
  const lockedItems = isPro ? [] : items.slice(effectiveLimit);
